...
# 截取方式3，截取codeforce
def extract_3(text: str) -> str:
    start = text.find("题意翻译")
    end = text.find("题目描述")
    if start != -1 and end != -1:
        return text[start + len("题目描述"):end]
    else:
        return ""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for p in paragraphs:
                    ...
                    str1 += p.text
                # .replace('\n', '\\n')
                data = "题目:" + remove_signs(extract_3(str1))
                data = data.replace('\n', ' ').replace('\r', ' ').replace('\"', ' ')
                data = data.replace('\\', '\\\\').replace('\u0009', ' ').replace('\t', ' ')
                if (contains_url(remove_signs(extract_2(str1))) == false and has_invalid_escape_sequences(
                        data) == False and 10 <= len(data) <= 100):
                    with open("codeforce100.json", 'a', encoding='utf-8') as f:
                        f.write(
                            "{\"content\": \"请出一道编程算法题目，只出题干，不包括解题过程,格式一致\",\"summary\": \"" + data + "\"}" + "\n")
                        f.close()
                elif (contains_url(remove_signs(extract_2(str1))) == false and has_invalid_escape_sequences(
                        data) == False and 101 <= len(data) <= 200):
                    with open("codeforce200.json", 'a', encoding='utf-8') as f:
                        f.write(
                            "{\"content\": \"请出一道编程算法题目，只出题干，不包括解题过程,格式一致\",\"summary\": \"" + data + "\"}" + "\n")
                        f.close()
                elif (contains_url(remove_signs(extract_2(str1))) == false and has_invalid_escape_sequences(
                        data) == False and 201 <= len(data) <= 300):
                    with open("codeforce300.json", 'a', encoding='utf-8') as f:
                        f.write(
                            "{\"content\": \"请出一道编程算法题目，只出题干，不包括解题过程,格式一致\",\"summary\": \"" + data + "\"}" + "\n")
                        f.close()
                elif (contains_url(remove_signs(extract_2(str1))) == false and has_invalid_escape_sequences(
                        data) == False and 301 <= len(data)):
                    with open("codeforce301x.json", 'a', encoding='utf-8') as f:
                        f.write(
                            "{\"content\": \"请出一道编程算法题目，只出题干，不包括解题过程,格式一致\",\"summary\": \"" + data + "\"}" + "\n")
                        f.close()
                else:
                    if contains_url(remove_signs(extract_3(str1))) == true: print(
                        "\033[31m[Warning] 存在url链接，数据已去除\033[0m")
                    if len(data) < 10: print("\033[31m[Warning] 字符数量太少，疑似提取错误\033[0m")
                # print("\n" + data)
                print("\n" + data)
                # print(contains_url(remove_signs(extract_3(str1))), has_invalid_escape_sequences(data))
                # print(s)
            else:
